...
function guardarEnHistorial(venta) {
...
function mostrarHistorial() {
    let historial = JSON.parse(localStorage.getItem("historialVentas")) || [];
    const contenedor = document.getElementById("lista-historial"); // Asegurate que este ID exista
    ...
    historial.forEach((venta, index) => {
        ...
        item.innerHTML = `
            ...
            <p><strong>Productos:</strong> ${venta.productos.map(p => `${p.nombre} x${p.cantidad}`).join(", ")}</p>
...
function guardarEnHistorialVenta(venta) {
    let historial = JSON.parse(localStorage.getItem("historialVentas")) || [];
    historial.push(venta);
    localStorage.setItem("historialVentas", JSON.stringify(historial));
}

document.getElementById("historial-tab").addEventListener("click", () => {
